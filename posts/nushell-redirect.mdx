...
While a deliberate design decision, not all POSIX shell operations can be translated directly to Nu.
...
Now, running the same command in Nu prints the following to stdout:
...
Interestingly, even though the `>` redirection operator doesn't exist in Nu, the command actually succeeds.
